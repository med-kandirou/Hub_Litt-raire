...
<script >
    ...
    methods:{
        ...
        async onSubmit(){
            ...
            await axios.post("https://api.cloudinary.com/v1_1/dxn7gskyn/auto/upload",formPdf)
            .then((res) => {
            ...
            livre.append("image", this.UrlImage)
            livre.append("file",this.UrlFile)
            livre.append("id_cat",this.selectedCat)

            axios.post('http://127.0.0.1:8000/api/livre/addLivre',livre)
            .then((res)=>{
                if(res.data="added"){
                        this.$swal.fire(
                            'Succes!',
